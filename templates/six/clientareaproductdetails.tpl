...
{* Pending Invoice Notification Strip *}
{if $pendingInvoice}
{include file="$template/includes/alert.tpl" type="warning" msg="You have an unpaid invoice of {$recurringamount} for this service. Pay now to enjoy uninterrupted service." textcenter=true}
{/if}
{* Pending Invoice Notification Strip *}
...
<div class="tab-content margin-bottom">
    <div class="tab-pane fade in active" id="tabOverview">
        ...
        {else}
          ...
          {* Pending Invoice Details, Invoice id, amount, and link to invoice payment page *}              
                        <h4>Service Invoice</h4>
                        {if $pendingInvoice}
        						<a href="viewinvoice.php?id={$pendingInvoice.invoiceId}">Invoice #{$pendingInvoice.invoiceId} <br /> Amount: {$pendingInvoice.invoiceTotal} {$pendingInvoice.clientCurrencyCode}</a>
						{else}
    						No Invoice Due
						{/if}
          {* Pending Invoice Details, Invoice id, amount, and link to invoice payment page *}   
